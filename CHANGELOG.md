...
- **A composed block can fan out inside the parent run.** An `include:` node now accepts `fan_out:` (#2512). Where a `workflow:` node with `fan_out:` gives every item its own governed child run — its own row, artifacts, cost line and gates — an `include:` node with `fan_out:` expands the statically named block once per item *inside this run*: one `workflow_runs` row, instance-qualified step names (`<nodeId>__<instanceId>__<innerNodeId>`) in the parent event stream, bounded by `max_parallel`, aggregated in input order into `$<id>.output`. Each instance is expanded at execution time through the same load-time machinery a static include uses — declared-input binding, command/script resolution from the block's own package, deterministic namespacing — so composition semantics are identical to the 1:1 case. Instances are keyed by a content hash of their item (plus a duplicate ordinal) and snapshotted onto the run before anything schedules, so resume threads completed instances by identity instead of re-running them even when the producer's list shrank, grew, or reordered. Two limits are enforced before any spend: the block may not contain approval gates (the parent's single slot cannot be held by N concurrent instances), and `isolation:` is rejected — instances share the parent checkout, so a mutating body must declare `mutates_checkout: false` or run with `max_parallel: 1`. The existing `workflow:` + `fan_out:` child-run path is unchanged; nothing is migrated or reinterpreted.

- **Workflow values keep their type across every boundary, and command/script nodes bind them by name.** A structured node result used to be faithfully typed only inside the run that produced it: persistence kept the text, a cold resume re-parsed it, every `with:` value was a string, and a command file or named script could only receive an upstream VALUE through an artifact file the producer wrote and the consumer re-read. Now the logical value rides along everywhere. `node_completed` events persist it beside the text and resume rehydrates it, so a resumed run's `$node.output.field` reads are identical to a fresh run's. `with:` maps and `inputs:` defaults accept any JSON value — `flag: true` reaches the child as a boolean, not `"true"`. A child run's terminal structured value threads back typed to its parent. And `command:`/`script:` nodes accept their own `with:` map, binding upstream values by name through the surfaces those bodies already read (`$INPUTS.<name>` in a command file, `INPUTS_<UPPER_SNAKE>` env vars in a script) — with an explicit `{ from, if_skipped }` form for reading across a skipped branch at an `all_done` join, so a mutually-exclusive-branches join no longer needs a touch-file latch. Bindings fail loudly: an unknown or non-upstream producer is a load error, and a skipped producer without `if_skipped` fails the node naming the fix — never a silent empty string. (#2637)
...
- **`archon complete <branch>` no longer requires `--force` for a never-pushed branch with no unique commits.** The completion safety check that flags a missing `<remote>/<branch>` ref treated "branch has never been pushed" as a blocker even when the branch provably carried no commits of its own (every commit already reachable from a surviving ref), forcing operators to use `--force` to clean up local-only branches. The "never pushed" message is now gated on the unique-commit check having returned a positive count: a branch that is byte-identical to the configured base completes without `--force`, while a branch that genuinely carries unpushed unique commits still blocks with the same message. If the unique-commit check itself fails (git error, permission denied, timeout), the gate is false and the existing fail-closed blocker — `could not determine unique commits — refusing to delete unverified` — still covers the unverifiable case. (#2433)
