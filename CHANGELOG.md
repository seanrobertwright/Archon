...
- **Dependency updates for `bun.lock` are automated again.** Dependabot had not completed a run on `dev` since 2026-04-07; every run failed with `dependency_file_not_supported` because GitHub's config-free security-update path lands on the `npm_and_yarn` updater, which cannot read `bun.lock`. That is not a misconfiguration a `.github/dependabot.yml` would fix — GitHub lists Bun as supported for version updates but **not** for security updates, so no ecosystem entry unlocks that path. A `renovate.json` now configures Renovate's dedicated `bun` manager, which reads the text `bun.lock` and the `packages/*` workspaces, with `lockFileMaintenance` enabled because this repository's dominant exposure is a declared range that already permits a patched version while the frozen lockfile resolves to a stale one — a class that produces no vulnerability alert at all, and that no alert-driven automation can find. **Two operator actions are required and cannot be done from the repository:** install the Renovate app, and turn Dependabot **security updates** off while leaving Dependabot **alerts** and the dependency graph **on** — Renovate reads that alert feed, alongside a second OSV-based feed that does not depend on this repository's alert setting. (#2666)
