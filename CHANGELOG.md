...
- **`/reset` is now a real escape hatch.** It used to deactivate the AI session and leave the conversation's execution binding (`cwd` + `isolation_env_id`) and every resumable run untouched, so the next message still resolved to the prior worktree and could silently continue a stale `paused`/`failed` run with its old arguments and completed nodes. `/reset` now nulls the binding and abandons the conversation's resumable runs through the same `abandonWorkflow` op every other surface uses, preserving `codebase_id` (detaching the project is `/setproject none`'s job) and deliberately leaving `pending`/`running` work alone — those rows may belong to a different process entirely (CLI run, webhook, scheduled dispatch) and the resume lookups this exists to defeat never matched them anyway. The abandon and the binding-clear sit in separate try blocks so a failure in the second surfaces the abandoned count from the first instead of swallowing it. (#2291)
