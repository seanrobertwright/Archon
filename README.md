...
- **Aggregate usage** — provider-reported gross input, output, optional cache-read/cache-write token totals, and cost (USD) per workflow run, plus direct-chat usage and total loop iterations. Numeric totals only — never the content the tokens represent.
