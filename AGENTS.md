...
- `--json` is supported on the read/write subcommands (`list`, `status`, `runs`, `get`, `approve`, `reject`, `respond`, `cancel`, `abandon`, `resume`). On `approve`/`reject`/`respond`/`resume` it records the decision and returns an ack **without** the inline auto-resume, leaving the run resumable — drive continuation separately.
- `workflow cancel <run-id>` actively stops a live CLI `--detach` owner by exact run id: it confirms the process tree is gone before recording `cancelled`, and leaves state unchanged when it cannot prove termination. `workflow abandon <run-id>` is deliberately state-only; use it for paused runs or after independently verifying that a running row is orphaned.
- `--detach` works on `approve`/`reject`/`respond`/`resume` as well as `run`. Adding it INVERTS the `--json` rule above: the parent validates the run read-only and refuses synchronously, then spawns a child without `--json`, so the child takes the inline path and DOES continue the run. The ack carries `continues: true` so an automation knows it no longer owns continuation. On `run`, `--detach` additionally refuses synchronously (before any fork) when the target workflow is interactive-class (#2707 step 2 / #1991) — this is a LAUNCH refusal only; `approve`/`reject`/`respond`/`resume --detach` are continuation actions on an already-paused run and are exempt.
- `workflow respond <run-id> <decision> [text]` (#2707 step 2) resolves a paused gate with any of its author-declared decisions; `approve`/`reject` remain sugar for the default vocabulary and keep their own dedicated subcommands.
...
- `/workflow` handles subcommands like `list`, `run`, `status`, `cancel`, `resume`, `abandon`, `approve`, `reject`, `respond`, `reset-sessions`
...
2. **Workflows** (YAML-based):
   ...
   - `interactive: true` at the workflow level is the workflow's CLASS declaration (#2707 step 2): "this workflow's tree may pause." A NATIVE gate/interactive-loop authored directly in an undeclared workflow's own DAG is checked at load time (`loader.ts`'s workflow-class placement check); a gate that arrives via `include:` composition is refused at invocation instead (`assertComposedGateDriveable`), since the same reusable block can be legitimately composed by different parents. During a grace period (#2736/#2738), an undeclared native violation does not reject the file — `interactive: true` is inferred for that parse and a warning is emitted once per file (log) plus on every load (`parseWarnings`); the check becomes a hard load error again once the grace period ends. An interactive-class workflow (declared OR inferred) can never be dispatched to the background, on any surface — web's default dispatch, the `manage_run` native tool, and CLI `workflow run --detach` each independently refuse it (`assertInteractiveClassNotBackgrounded`) — required for any approval-gate workflow, not just in the web UI
...
- `POST /api/workflows/runs/{runId}/approve` - Approve a paused workflow run; body `{ comment? }`
- `POST /api/workflows/runs/{runId}/reject` - Reject a paused workflow run; body `{ reason? }`
- `POST /api/workflows/runs/{runId}/respond` - Resolve a paused run with any of the gate's declared decisions (#2707 step 2); body `{ decision, text? }`. `approve`/`reject` produce the exact same resolution as the two dedicated routes above — they delegate to the same core functions — this route exists for a gate that declares decisions beyond that default pair.
