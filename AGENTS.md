...
2. **Workflows** (YAML-based):
   ...
   - `interactive: true` at the workflow level is the workflow's CLASS declaration (#2707 step 2): "this workflow's tree may pause." An unattended workflow (`interactive` absent/`false`) may never contain a pause node anywhere in its DAG — checked at load time (`loader.ts`'s workflow-class placement check) for a native gate/interactive-loop, and refused at invocation (`assertComposedGateDriveable`) for one that arrives via `include:`. An interactive-class workflow can never be dispatched to the background, on any surface — web's default dispatch, the `manage_run` native tool, and CLI `workflow run --detach` each independently refuse it (`assertInteractiveClassNotBackgrounded`) — required for any approval-gate workflow, not just in the web UI
