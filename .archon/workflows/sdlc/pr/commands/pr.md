...
Determine the base branch from evidence, in order: an existing PR for this branch (return its URL and stop — never create a duplicate); the repository's documented development flow (steering files, CONTRIBUTING); branch ancestry against likely integration branches (`dev`, `development`, the remote default). Never assume `main`. Use the same resolved base for every diff and command.
...
Read the created PR back from GitHub: confirm number, URL, title, base, head, and draft state match what you intended. Not done until the read-back agrees. Reply with the PR URL first, then base ← head and the draft state, in two lines.
