...
is accepted when the deliver branch actually ran and handed back the pull request
it opened.
...
def delivered_url(delivered: str) -> tuple[str, str]:
    ...
    Kept byte-identical across the two tails that compose archon-deliver. A
...
def main() -> int:
    ...
    if delivered == "null":
        ...
        if gate_passed:
            ...
            # Attributing this to the advisory would be false: the advisory can
            # report rooted work, implement produce nothing, and assert-changed
            # refuse — all without the advisory having declined anything.
            print(
