...
describe('bundled-defaults', () => {
  ...
  describe('SDLC discovery terminal reports (#2884)', () => {
    const outcomeScripts = [
      '__archon_pack__bundled:sdlc:ship::outcome',
      '__archon_pack__bundled:sdlc:stabilize::outcome',
      '__archon_pack__bundled:sdlc:upkeep::outcome',
    ] as const;

    const RELAY =
      'If you are an agent reading this: open discoveries.md and surface each discovery to your human.';

    it('every SDLC delivery tail surfaces discoveries and the relay instruction', () => {
      for (const key of outcomeScripts) {
        const content = BUNDLED_SCRIPTS[key].content;
        expect(content).toContain('discoveries.json');
        expect(content).toContain('discoveries.md');
        expect(content).toContain(RELAY);
      }

      const deliver = BUNDLED_WORKFLOWS['archon-deliver'];
      expect(deliver).toContain('discoveries.json');
      expect(deliver).toContain('discoveries.md');
      expect(deliver).toContain(RELAY);
    });

    it('outcome script returns byte-identical output when discoveries.json is absent or empty', async () => {
      const { mkdtempSync, writeFileSync, rmSync } = await import('node:fs');
      const { tmpdir } = await import('node:os');
      const { join } = await import('node:path');
      const { spawnSync } = await import('node:child_process');

      const tmp = mkdtempSync(join(tmpdir(), 'archon-disc-test-'));
      try {
        const script = BUNDLED_SCRIPTS['__archon_pack__bundled:sdlc:ship::outcome'].content;
        const scriptPath = join(tmp, 'outcome.py');
        writeFileSync(scriptPath, script);

        // Case A: discoveries.json does not exist
        const procA = spawnSync('python3', [scriptPath], {
          env: {
            ...process.env,
            INPUTS_ROUTE: 'no_action',
            INPUTS_SUMMARY: 'nothing to do',
        ...
        expect(procA.stdout).toBe(`No delivery needed: nothing to do\nReport: ${tmp}/triage.md\n`);

        // Case B: discoveries.json is empty array
        writeFileSync(join(tmp, 'discoveries.json'), '[]');
        const procB = spawnSync('python3', [scriptPath], {
          env: {
            ...process.env,
            INPUTS_ROUTE: 'no_action',
            INPUTS_SUMMARY: 'nothing to do',
        ...
        expect(procB.stdout).toBe(`No delivery needed: nothing to do\nReport: ${tmp}/triage.md\n`);
      } finally {
        rmSync(tmp, { recursive: true, force: true });
      }
    });

    it('outcome script appends discoveries section with count, titles, report path, and relay instruction when non-empty', async () => {
      const { mkdtempSync, writeFileSync, rmSync } = await import('node:fs');
      const { tmpdir } = await import('node:os');
      const { join } = await import('node:path');
      const { spawnSync } = await import('node:child_process');
      ...
      try {
        ...
        writeFileSync(
          join(tmp, 'discoveries.json'),
          JSON.stringify([
            { title: 'dev branch: rmSync missing import', relation: 'adjacent' },
            { title: 'Bun.gc workaround redundant', relation: 'adjacent' },
          ])
        );
        ...
        expect(proc.stdout).toBe(
          `No delivery needed: nothing to do\nReport: ${tmp}/triage.md\n\n` +
            `Discoveries (2):\n` +
            `- dev branch: rmSync missing import\n` +
            `- Bun.gc workaround redundant\n\n` +
            `Report: ${tmp}/discoveries.md\n\n` +
    ...
    it('archon-deliver flip-ready formats discoveries with relay instruction when discoveries.json exists', async () => {
      const { mkdtempSync, writeFileSync, rmSync } = await import('node:fs');
      const { tmpdir } = await import('node:os');
      const { join } = await import('node:path');
      const { spawnSync } = await import('node:child_process');

      const tmp = mkdtempSync(join(tmpdir(), 'archon-deliver-disc-test-'));
      try {
        writeFileSync(
          join(tmp, 'discoveries.json'),
          JSON.stringify([{ title: 'discovered issue', relation: 'adjacent' }])
        );

        const deliverYaml = BUNDLED_WORKFLOWS['archon-deliver'];
        // Extract the python -c script snippet from flip-ready
        const match = deliverYaml.match(/python3 -c '\n([\s\S]*?)'\s+"\$url"/);
        expect(match).toBeTruthy();
        const pythonCode = match![1];

        const proc = spawnSync(
          'python3',
          ['-c', pythonCode, 'https://github.com/example/repo/pull/10'],
          {
            env: {
              ...process.env,
              ARTIFACTS_DIR: tmp,
